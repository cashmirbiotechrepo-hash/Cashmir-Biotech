...
async function main() {
  // HIGH-09: Production guard to protect live database from accidental wipes/overwrites
  if (process.env.NODE_ENV === "production") {
    throw new Error("Seed script is blocked in production to protect live data!");
  }
  ...
  // HIGH-09: Replaced deleteMany({}) with upsert() so existing data isn't wiped
  const products = [
    {
