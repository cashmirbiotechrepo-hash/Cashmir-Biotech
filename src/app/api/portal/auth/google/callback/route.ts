...
export async function GET(request: Request) {
  ...
  if (errorParam) {
    ...
    return NextResponse.redirect(new URL("/portal/login?error=oauth_denied", request.url));
  }

  if (!code) {
    return NextResponse.redirect(new URL("/portal/login?error=oauth_missing_code", request.url));
  }
  ...
  if (!statePayload) {
    ...
    return NextResponse.redirect(new URL("/portal/login?error=oauth_state_mismatch", request.url));
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.redirect(new URL("/portal/login?error=oauth_token_failed", request.url));
    }
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.redirect(new URL("/portal/login?error=oauth_userinfo_failed", request.url));
    }
  ...
  if (!userInfo.email_verified) {
    ...
    return NextResponse.redirect(new URL("/portal/login?error=oauth_unverified_email", request.url));
  }
  ...
  try {
    ...
    if (existingAccount) {
      ...
      if (existingCustomer) {
        if (!existingCustomer.active) {
          ...
          return NextResponse.redirect(new URL("/portal/login?error=account_inactive", request.url));
        }
    ...
    return NextResponse.redirect(new URL("/portal/login?error=oauth_server_error", request.url));
  }
